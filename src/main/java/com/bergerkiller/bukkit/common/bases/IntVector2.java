...
public class IntVector2 implements Cloneable {
    ...
    public IntVector2 multiply(int x, int z) {
    	IntVector2 newVector = null;
    	try 
    	{
    		newVector = ((IntVector2)this.clone());
    	} 
    	catch (CloneNotSupportedException e) 
    	{
    		e.printStackTrace();
    	}
    	newVector.setX(newVector.getX() * x);
    	newVector.setZ(newVector.getZ() * z);
    	return newVector;
    }
    ...
    /**
     * Checks whether the x or z coordinate is greater/equal than the value
     * specified
     *
     * @param value to check
     * @return True if x/z >= value, False otherwise
